...
description: Chain bug, security, technical-debt, process-debt, repair, and validation prompts over a completed change, reviewable slice, or cumulative branch diff. Use during iterative implementation, before a release or commit, or whenever the user requests a comprehensive change audit and repair without durable audit records.
---
...
5. **Verify and repair:** Confirm findings against the code, discard false positives and unrelated pre-existing debt, then fix valid in-scope root causes. Recheck repaired areas until no verified blocker remains.
