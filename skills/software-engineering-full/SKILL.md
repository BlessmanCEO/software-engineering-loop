...
Invoke chained skills with the active harness's syntax: `$skill-name` in Codex or `/skill:skill-name` in Pi.
...
- Pi commit review: `pi -p --no-session "Review commit <commit-sha>. Inspect it with git show. Do not modify the repository. Return only verified findings, ordered by severity."`
- Pi base review: `pi -p --no-session "Review all changes since <base-branch>. Inspect them with git diff <base-branch>...HEAD. Do not modify the repository. Return only verified findings, ordered by severity."`
