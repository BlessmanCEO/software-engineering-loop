...
def resume_status(args) -> None:
    ...
    if state["status"] in {"passed", "degraded"} and state.get("finalSha"):
        next_action = "complete"
    elif blocked or state["status"] == "blocked":
    ...
    elif state.get("validationLease"):
        next_action = f"recover {state['validationLease']['phase']} lease"
    else:
        incomplete = next(
        ...
        if incomplete:
            ...
            if record.get("validation") != "pass":
                next_action = f"record worker validation for {name}"
            elif record["status"] == "planned":
                next_action = f"acquire-writer {name}"
            elif record.get("review", {}).get("status") != "pass":
                next_action = f"unified slice review for {name}"
            else:
